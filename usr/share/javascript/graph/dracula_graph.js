...
Graph.Renderer.Raphael.prototype = {
    ...
    draw: function() {
        ...
        for(var i in this.graph.nodes){
        	if(typeof this.graph.nodes[i] != 'function')
            this.drawNode(this.graph.nodes[i]);
        }
        for(var i = 0; i < this.graph.edges.length; i++){
			if(typeof this.graph.edges[i] != 'function')
            this.drawEdge(this.graph.edges[i]);
        }
		for(var i in this.graph.nodes){
        	if(typeof this.graph.nodes[i] != 'function')
            	if(typeof Graph.inList[this.graph.nodes[i].id] == 'undefined')
            		this.graph.nodes[i].shape.items[0].attr({'fill': '#EC1A1F', 'stroke': '#E29E9F'});
        }
    },
    ...
    drawNode: function(node) {
        ...
        /* the default node drawing */
            ...
            shape = this.r.set().
                push(this.r.circle(point[0], point[1], 15).attr({fill: (node.edges.length > 0 ? '#F1D556' : '#33D069'), stroke: (node.edges.length > 0 ? '#B1A25B' : "#1AEC62"), "stroke-width": 2, title: node.label || node.id})).
                push(this.r.text(point[0], point[1], node.label || node.id));
        }
		tooltip = this.r.set().push(
			this.r.rect(-20, -15, 250, 30).attr({"fill": "#000", "stroke-width": 1, r : "9px"})
		).push(
			this.r.text(100, 0, node.tooltip || "testing").attr({"fill": "#fff"})
		);
		for(i in shape.items){
			if(typeof shape.items[i] != 'function')
        	shape.items[i].tooltip(tooltip);
        };
        shape.attr({"fill-opacity": 1});
        var intervals = [];
        shape.items[1].node.onmouseover = function(){
        	for(var i in node.edges){
        		if(typeof node.edges[i] != 'function'){
        			node.edges[i].connection.fg.attr({"stroke": "red",  "stroke-width": 3})
        			node.edges[i].target.shape.items[1].attr({'stroke': '#5F0303'});
        			node.edges[i].target.shape.items[0].attr({"opacity": 1.0}).animate({"opacity": 0.5}, 100, function(){
        				this.animate({"opacity": 1.0, "stroke-width": 6}, 100, function(){
        					this.animate({"opacity": 0.2, "stroke-width": 3}, 100, function(){
        						this.animate({"opacity": 0.2, "stroke-width": 6}, 00, function(){
									this.animate({"opacity": 1.0, "stroke-width": 3}, 100, function(){
        	...
        	if(Graph.inList[node.id]){
        		for(var i in Graph.inList[node.id]){
        			for(var j in Graph.inList[node.id].edges){
        				if(typeof Graph.inList[node.id].edges[j] != 'function')
        					Graph.inList[node.id].edges[j].connection.fg.attr({"stroke": "blue",  "stroke-width": 2});
        			}
        		}
			}
			
        }
 		shape.items[1].node.onmouseout = function(){
        	for(i in node.edges){
        		if(typeof node.edges[i] != 'function'){
        			node.edges[i].connection.fg.attr({"stroke": "#000", "stroke-width": 1});
        		}
        	}
        	if(Graph.inList[node.id]){
        		for(var i in Graph.inList[node.id]){
        			for(var j in Graph.inList[node.id].edges){
        				if(typeof Graph.inList[node.id].edges[j] != 'function')
        					Graph.inList[node.id].edges[j].connection.fg.attr({"stroke": "#000", "stroke-width": 1});
        			}
        		}
			}
        }
    ...
    drawEdge: function(edge) {
        ...
        if(!edge.connection) {
            ...
            var fromNodeName = edge.target.shape.items[1].attr('text');
            if(Graph.inList[fromNodeName]){
            	Graph.inList[fromNodeName].count++;
            	Graph.inList[fromNodeName].edges.push(edge);
            }else{
            	Graph.inList[fromNodeName] = {
            		count: 1,
            		edges: [edge],
            	};
            }
