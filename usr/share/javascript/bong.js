...
//Bong Wrapper
var bong = {
	bind:    function(url, fnc){
		jx.bind({
			url: url,
			onSuccess: fnc
		})
	},
	update:  function(url, dom){
		jx.bind({
			url: url,
			method: 'text',
	...
	bindInterval: function(url, fnc, interval){
		return setInterval(interval, this.bind(url, fnc));
	},
	refresh: function(url, dom, interval){
		return setInterval(interval, this.update(url, dom));
	}
};